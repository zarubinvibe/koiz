...
const MUTS = [
  ...
  ['koiz.mjs', "const act = active(st).filter(l => l.why || l.fix || l.op === 'rule')", 'const act = active(st).filter(l => true)', 'сырьё без причины идёт в обобщение'],
  ['koiz.mjs', 'const [stary, mladshy] = [act[i], act[j]]', 'const [mladshy, stary] = [act[i], act[j]]', 'живым остаётся старший, а не младший'],
  ['koiz.mjs', 'process.exitCode = code', 'process.exit(code)', 'большая выдача обрывается на выходе'],
  ['koiz.mjs', "const positionals = () => args.filter((a, i) => !a.startsWith('--') && !(i > 0 && VALUE_FLAGS.has(args[i - 1])))", "const positionals = () => args.filter(a => !a.startsWith('--'))", 'значение ключа принимается за запрос'],
  ['koiz-capture.mjs', 'if (!what || NOISE.test(what) || EXIT_ONLY.test(what) || THIN(what)) return null', 'if (false) return null', 'наблюдатель пишет шум сторожей'],
  ['koiz.mjs', "const known = guard(repro ? `${repro} ${obs.what}` : obs.what, { db, limit: 1 })", 'const known = []', 'починка из базы не возвращается в момент падения'],
  ['koiz-capture.mjs', 'красный:|FAIL\\s|', 'красный:|FAIL\\b|', 'маркер провала ловит счетчики вида FAIL=0'],
  ['koiz-capture.mjs', 'if (NOISE.test(String(what)) || EXIT_ONLY.test(String(what)) || THIN(what)) return', 'if (false) return', 'работа сторожа записывается уроком'],
  ['koiz-capture.mjs', 'NOISE.test(what) || EXIT_ONLY.test(what)', 'NOISE.test(what) || false', 'ненулевой код принимается за беду'],
